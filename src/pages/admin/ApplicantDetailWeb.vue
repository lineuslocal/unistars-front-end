<template>
  <q-page style="margin-top:30px">
    <div class="cus-title-table">
      <q-icon name="perm_contact_calendar"/>&nbsp;&nbsp;&nbsp;Applicant - Detail
    </div>
    <div class="cus-form">
      <div class="row">
        ...
        <div class="offset-sm-1 col-sm-8 offset-0 col-12 row">
          ...
          <div v-if="applicant.addInfors.length > 0" style="max-width: 700px;" class="col-12">
            <div class="col-11" style="border-bottom: 2px solid rgba(0, 0, 0, 0.12); margin-bottom:14px"></div>
            <q-item v-for="(infor, index) in applicant.addInfors" :key="index" style="padding:0;" class="col-12">
              <p class="col-5 cus-text">{{infor.question}}<span v-if="infor.isRequired" >*</span></p>
              <p class="offset-1 col-6" >{{infor.answer}}</p>
            </q-item>
          </div>
          <div style="margin-bottom:16px; max-width: 700px" class="col-12 row" >
            ...
            <p class="col-12 cus-text" style="margin-bottom:5px" v-if="applicant.surveys.length > 0">Pull Results</p>
    ...
    <div class="text-center" style="margin-top:15px">
        <q-btn label="OK"  color="primary" :to="'/admin/event/list/' + this.$route.params.cat_id + '/applicant/list/' + this.$route.params.event_id "/>
    </div>
  </q-page>
</template>
